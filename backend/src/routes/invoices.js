...
router.get('/', auth, async (req, res) => {
  const { rows } = await pool.query(`
    SELECT i.*, c.name as customer_name,
      json_agg(DISTINCT jsonb_build_object('id',ii.id,'product_id',ii.product_id,'qty',ii.qty,'price',ii.price,'disc',ii.disc,'remark',ii.remark,'product_name',p.name,'product_name_kh',p.name_kh)) as items,
